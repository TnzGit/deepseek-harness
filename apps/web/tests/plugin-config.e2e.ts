...
describe('web e2e: plugin configuration pages', () => {
  ...
  it('saves subagent limits and resets them to the deployment defaults', async () => {
    ...
    const capacity = panel.getByLabel('可续接子智能体驻留上限', { exact: true })
    const concurrent = panel.getByLabel('一次性子智能体并发运行上限', { exact: true })
    expect(await depth.inputValue()).toBe('1')
    expect(await capacity.inputValue()).toBe('8')
    expect(await concurrent.inputValue()).toBe('8')
    await depth.fill('2')
    ...
    await expect.poll(settingsDocument).toContain('maxConcurrentRuns: 2')
  ...
  it('opens field explanations with the keyboard and retains unsaved edits', async () => {
    ...
    await panel.getByRole('button', { name: '可续接子智能体驻留上限说明', exact: true }).click()
    const capacityRules = panel.getByRole('region', { name: '可续接子智能体驻留上限说明', exact: true })
