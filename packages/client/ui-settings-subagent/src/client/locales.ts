...
export const en: Record<SubagentSettingsLocaleKey, string> = {
  ...
  subagentConcurrentHelp: 'Maximum one-shot Subagent runs executing at the same time across providers. Additional root-Agent starts wait in FIFO order; nested Subagent starts fail fast at capacity to avoid parent/child deadlocks. Set this to 1 on a single local inference engine when parallel contexts cause KV-cache contention.',
...
export const zh: Record<SubagentSettingsLocaleKey, string> = {
  ...
  subagentConcurrentHelp: '限制所有 provider 同时执行的一次性子智能体数量。根 Agent 的超额启动请求按 FIFO 排队；嵌套子智能体在容量已满时立即失败，避免父子互相等待造成死锁。单个本地推理引擎在并行上下文导致 KV 缓存争用时，可将此值设为 1。',
