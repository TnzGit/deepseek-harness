...
Unary JSON 响应在 API carrier 内仍是普通 Fetch 响应。到 node:http bridge 后，只有正文大于 1 KiB 且客户端声明支持相应编码时才压缩：Brotli（`br`）的 quality 不低于 gzip 时优先使用 Brotli，否则使用 gzip。已有 `Content-Encoding` 不会被覆盖；压缩响应会设置 `Content-Length` 并在 `Vary` 中加入 `Accept-Encoding`；小型 JSON 保持不压缩。非 JSON 与流式响应不会进入这条缓冲路径，因此 WebSocket 下行以及任何 SSE 响应都继续保留既有的增量写入与 backpressure 行为。
