...
function captureResponse(): {
  ...
  result(): { status: number | undefined; headers: Record<string, string>; body: Buffer }
} {
...
function jsonRequest(headers: Record<string, string> = {}): IncomingMessage {
...
describe('HTTP bridge JSON compression', () => {
  it('prefers Brotli for JSON larger than 1 KiB when the client supports it', async () => {
    const captured = captureResponse()
    await bridge(jsonRequest({ 'accept-encoding': 'gzip, br' }), captured.response, {
      requestBodyMode: () => 'buffered',
    ...
    expect(JSON.parse(brotliDecompressSync(result.body).toString('utf8')))
      .toEqual({ text: 'x'.repeat(4_000) })
  })

  it('uses gzip when Brotli is disabled by the client quality value', async () => {
    const captured = captureResponse()
    await bridge(jsonRequest({ 'accept-encoding': 'br;q=0, gzip;q=0.8' }), captured.response, {
      requestBodyMode: () => 'buffered',
    ...
    expect(JSON.parse(gunzipSync(result.body).toString('utf8')))
      .toEqual({ text: 'y'.repeat(4_000) })
  })

  it('leaves small JSON responses uncompressed', async () => {
    const captured = captureResponse()
    await bridge(jsonRequest({ 'accept-encoding': 'br, gzip' }), captured.response, {
      requestBodyMode: () => 'buffered',
