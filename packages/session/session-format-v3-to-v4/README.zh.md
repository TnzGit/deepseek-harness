...
- **已接受 V4 转换**——[检查点](../../../docs/session-format-status.zh.md#finalization-record)保护已接受历史。当前写入器使用 [V4 到 V5 后继版本](../session-format-v4-to-v5/README.zh.md)；已写入的 V4 文件不会重跑此入边，历史输入保持不变。
- **V4 前置读取器**——V4 子日志证据使用固定代际的 V4 目录，而不经过已安装写入器的迁移。导出的 V4 恢复器提供代际自有检查；子证据读取器不会执行历史子 Session 的工具或重放其 Agent。
