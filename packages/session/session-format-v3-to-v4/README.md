...
- **Accepted V4 transition** — the [checkpoint](../../../docs/session-format-status.md#finalization-record) protects the accepted history. The current writer uses the [V4-to-V5 successor](../session-format-v4-to-v5/README.md); already-written V4 files do not rerun this incoming edge, and historical inputs remain intact.
- **V4 prerequisite reader** — V4 child evidence uses the fixed-generation V4 catalog rather than migrating through the installed writer. The exported V4 restorer supplies generation-owned checks; the child-fact reader does not execute a historical child's tools or replay its Agent.
