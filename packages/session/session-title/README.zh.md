...
fork 出的会话会原样继承种子中的标题事件。首消息节奏不会自动为子会话重新生成标题；全消息与每 N 条节奏可以在子会话收到后续用户提示词后追加新修订。
...
提供方会提供带品牌类型的稳定 id、自动模式（`first-prompt`、`all-prompts`，或附带必填正整数 `promptInterval` 的 `every-nth-prompt`）和 `generate(request)`。在 `every-nth-prompt` 节奏下，第 1 条提示词立即命名，此后每累计 `promptInterval` 条符合条件的新提示词调度一次修订。请求携带活跃会话、截至一次固定修订的所有符合条件消息、可用时当前已记录的主请求路由，以及取消信号。结果包含非空标题、该请求中互不重复且有序的来源消息 seq，以及生成该标题时使用的可选提供方／模型路由。服务会在结果持久保存前进行规范化和验证。
