...
- **Routed policy** — proactive pressure resolves capacity and the effective output reservation from the adapter that owns the latest durable provider/model route, then scales the default policy plus an optional exact-target override into concrete token budgets. Its input threshold is the lower of the configured ratio and `contextWindow − outputReservation − recountMargin`, so a large completion cap cannot silently consume the pressure headroom. Model discovery remains advisory and is not consulted.
...
- **Summarization** — a direct `llm/stream` call uses the configured provider/model pair and cap, falling back to the latest logged request target and then the agent target, without running the loop-only `agent/request` extension point. The call replays the conversation's own system prompt, tools, and shadowed-region messages verbatim, including image references, and appends the compaction instruction as the final user message, so it reuses the provider's warm prefix cache instead of invalidating it. The selected adapter must resolve or explicitly reject those images. It sets `GenerateOptions.purpose` to `compaction`; the built-in adapters disable model reasoning for that purpose, while the DeepSeek adapter also sends `x-deepseek-harness-compact: 1`. Only returned text enters the checkpoint, excluding reasoning and tool calls that would leak private reasoning or create an orphaned call; image output fails with `UNSUPPORTED_CONTENT` rather than disappearing.
...
- **Overflow recovery** — provider-confirmed overflow needs no capacity metadata: it bypasses normal pressure and retention, prunes, then attempts one maximal balanced head reduction while leaving the newest indivisible unit. A changed surface authorizes retry only when token-meter remeasurement proves positive relief; when the provider reported input and output reservations, the relief must cover the exact deficit plus the same recount margin used by adapter cap adaptation. This also governs pruning that lands before later summary work throws. No replacement, insufficient relief, an exhausted target-specific cap, cancellation, or an unknown/noncanonical error preserves the original provider failure.
...
|---|---|---|
| `thresholdRatio` | no (default `0.8`) | Input-pressure ceiling. The effective threshold is the lower of `floor(routedContextWindow × ratio)` and the capacity left after the effective output reservation plus recount margin. |
...
After a successful step crosses the output-aware threshold, oversized tool results are first rewritten when the optional pruner is loaded. If summarization remains necessary, the next request receives the checkpoint preamble below, a blank line, `<compacted-summary>`, the data-dependent summary, and `</compacted-summary>`. Overflow recovery rebuilds the immediate retry only after a replacement frees enough measured input. A checkpoint replaces the selected older range and is followed by the retained recent units.
...
- **Summarization failure preserves the latest durable surface** — before any replacement, the auto path logs a warning and proceeds with full over-budget history. If pruning already landed, a later summarization failure proceeds from that durable pruned surface and retries only when it freed enough measured input. Summarization-purpose calls disable supported reasoning modes so hidden reasoning cannot consume the checkpoint cap; truncation at `maxTokens` and successful streams with no substantive text both retry over progressively smaller balanced ranges before following the same fail-closed rule.
