...
- **路由策略**：主动压力从拥有最新持久提供方／模型路由的适配器解析容量和有效输出预留，再将默认策略与可选的精确目标覆盖缩放为具体 token 预算。输入阈值取配置比例与 `contextWindow − outputReservation − recountMargin` 两者中的较小值，因此较大的补全上限不会静默占掉压力余量。模型发现仍仅供参考，不参与此处的策略解析。
...
- **摘要**：直接 `llm/stream` 调用使用已配置的提供方／模型对与上限，回退到最新已记录请求目标，然后再回退到 agent（智能体）目标，而不运行仅用于 agent loop 的 `agent/request` 扩展点。该调用会逐字回放会话自身的系统提示词、工具与已遮蔽区域消息（包括图片引用），并将压缩指令作为最后一条 user 消息追加，从而复用提供方的热前缀 cache，而非使它失效。所选适配器必须解析或明确拒绝这些图片。它将 `GenerateOptions.purpose` 设为 `compaction`；内置适配器会针对此用途禁用模型推理，DeepSeek 适配器还会发送 `x-deepseek-harness-compact: 1`。只有返回的文本会进入检查点；推理（reasoning）和工具调用都会被排除，以免泄露私有推理或产生遗留调用；图片输出会以 `UNSUPPORTED_CONTENT` 失败，而不是消失。
...
- **溢出恢复**：提供方已确认的溢出不需容量元数据。它会绕过常规压力与保留，执行剪枝，再尝试一次最大平衡头部缩减，并留下最新不可分单元。只有表层发生变化且 token meter 重新计量证明 token 数确实减少时才允许重试；若提供方报告了输入和输出预留，减少量还必须覆盖精确缺口，以及适配器上限自适配使用的同一重新计数余量。这也适用于剪枝先落地而后续摘要工作抛错的情况。如果没有替换、解除量不足、目标特定上限已耗尽、已取消，或遇到未知／非规范错误，则保留原始提供方失败。
...
| Key | 必填 | 含义 |
|---|---|---|
| `thresholdRatio` | 否（默认 `0.8`） | 输入压力上限。有效阈值取 `floor(routedContextWindow × ratio)` 与扣除有效输出预留及重新计数余量后的容量两者中的较小值。 |
...
成功步骤越过考虑输出预留的阈值后，如果已加载可选 pruner，超大工具结果会先被改写。如果仍需摘要，下一个请求会收到下方检查点前导、一个空行、`<compacted-summary>`、根据数据生成的摘要以及 `</compacted-summary>`。只有替换释放了足够的实测输入，溢出恢复才会重建立即重试。检查点会替换已选较早范围，后面跟随已保留的近期单元。
...
- **摘要失败会保留最新持久表层**：任何替换前，自动路径会记录警告，并携带完整超预算历史继续。如果剪枝已落地，后续摘要失败会从该持久剪枝表层继续，并且只有它释放了足够的实测输入时才重试。摘要用途调用会禁用受支持的推理模式，以免隐藏推理耗尽检查点额度；达到 `maxTokens` 的截断及没有实质文本的成功流都会在逐步缩小的平衡范围上重试，之后才遵循同一故障关闭规则。
