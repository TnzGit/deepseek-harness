...
| `summaryRangeRetries` | `3` | 摘要触及输出上限时，缩小至更短且工具配对安全的范围后重试的次数；`0` 关闭此回退。 |
...
最旧的平衡范围会被替换为一条摘要消息，近期尾部保持逐字不变；对话从摘要继续。如果摘要触及输出上限，后端最多按 `summaryRangeRetries` 次缩小至更短的配对安全前缀后重试；失败尝试只留下已闭合的诊断标记，不会写入半截摘要。操作会报告压缩了多少历史项以及估算释放的 token 数。如果没有任何内容可以安全压缩——例如整个对话就是一个不可分单元——则不会有任何改变，也不会向会话日志写入任何内容。如果没有模型可以撰写摘要（既未配置目标，也还没有已路由请求），压缩会失败并给出清晰错误，提示你配置摘要提供方与模型，或先路由一次请求。
