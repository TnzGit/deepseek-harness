...
export type CompactionTrigger = 'pressure' | 'request-budget' | 'context-overflow'
...
export abstract class CompactionEngine extends Service {
   ...
   * uses the latest durable routed request; request-budget policy reserves the
   * resolved output cap before provider dispatch; context-overflow policy may
   * force a useful balanced reduction even below the normal threshold. Return
   * `null` when no safe range can be compacted. A single oversized retained
   * unit or request envelope cannot be repaired through surface compaction.
   *
   * @param agent - agent context owning the session surface and routing options.
   * @param trigger - normal pressure, request-output headroom, or provider-confirmed context overflow.
