...
interface Config {
  ...
  maxTokenContinuations?: number // default 4; 0 disables automatic continuation
  maxTokenContinuationOutputTokens?: number // default 163840; cumulative per turn
  agents: Array<{
    id: string                 // required
    provider?: string
    model?: string
...
A `max-tokens` finish whose committed message contains only private reasoning opens a bounded continuation step instead of ending the turn immediately. The durable `agent/max-token-continuation` event reconstructs the request by replacing that reasoning block with a hidden assistant text checkpoint before appending the internal recovery prompt, so provider templates cannot discard the working state. Pressure compaction may shadow the source message; reconstruction then keeps the compaction checkpoint instead of restoring the full reasoning. The request follows the ordinary path without overriding provider, model, `maxTokens`, thinking, or reasoning effort. Visible partial answers and tool calls never trigger this path. Count, cumulative-output, and exact-repeat guards end exhausted recovery with `max-tokens` plus `autoContinuation: 'exhausted'` so clients can explain that automatic recovery already ran ([decision](../../../.agents/notes/implemented/bug-fix/2026-08-29-reasoning-only-max-token-continuation.md)).
