...
interface Config {
  ...
  maxTokenContinuations?: number // default 1; 0 disables automatic continuation
  agents: Array<{
    id: string                 // required
    provider?: string
    model?: string
...
A `max-tokens` finish whose committed message contains only private reasoning opens one bounded continuation step instead of ending the turn immediately. The durable `agent/max-token-continuation` event reconstructs an internal user prompt that asks the model to continue from the committed reasoning and produce a visible answer or tool call. The new request follows the ordinary request path without overriding provider, model, `maxTokens`, thinking, or reasoning effort. Visible partial answers and tool calls never trigger this path. If the configured continuation budget is exhausted, the turn ends with `max-tokens` plus `autoContinuation: 'exhausted'` so clients can explain that automatic recovery already ran ([decision](../../../.agents/notes/implemented/bug-fix/2026-08-29-reasoning-only-max-token-continuation.md)).
