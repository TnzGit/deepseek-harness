...
- pi-ai's `off` thinking level crosses the Harness capability seam unchanged and becomes an omitted pi-ai common `reasoning` option at dispatch. Compaction-purpose calls force this level when the model supports it, preventing private reasoning from consuming the summary output cap.
