...
`defaultReasoningEfforts` carries the same declaration at route level, but only as the final fallback for a model absent from the installed catalog. Resolution is exact-model declaration → installed catalog capability → route fallback → no reasoning metadata. This keeps catalog facts authoritative while allowing a self-hosted route whose model id changes between deployments to retain its known protocol capability. The Models settings card edits both the route fallback and each model's override as inherit, disabled, or enabled with an exact level set; the selected effort remains the composer's per-session setting and the profile's optional `reasoning` default.
...
- **Copying the previous model's declaration when discovery returns a new id.** A neighboring row is not evidence about the new model, and the protection would disappear when the old row is removed. An explicit route fallback records the operator's endpoint knowledge once.
...
- #1860's deferred gap — a route-level effort a model cannot take failing its requests — has an operator remedy in the Models page: align the exact model's `reasoningEfforts`, define the route fallback for uncatalogued ids, or remove the selected default.
