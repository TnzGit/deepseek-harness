---
description: "Complete Session V4 persistence types, source evidence, and physical encoding references."
...
Inspect the V4 persistence types captured before the V5 writer change in the fork integration branch. V4 introduced tool-role results, native developer messages, and strict lifecycle admission; this snapshot retains the declared types of that checkpoint.
...
The source is the V4-writing integration tree in PR #7 before the V5 writer change. The V4 schema was archived from that isolated checkout without changing its declarations. This checkpoint is not a product release; the [release-status reference](../../session-format-status.md) owns publication evidence.

<a id="characteristics"></a>
## Format characteristics

The logical and physical headers use version 4. V4 stores tool results as tool-role messages and admits developer messages and producer-owned source attribution. Its physical rows retain the V2 framing, while [V4 codec admission](../../../packages/session/session-format-v3-to-v4/README.md) validates the added semantics. The [V4-to-V5 migration](../../../packages/session/session-format-v4-to-v5/README.md) advances the generation without rewriting historical events.

<a id="declaration"></a>
## Declaration

[Schema inventory](v4.schema.json).
...
| `schedule` | required | [`packages/schedule/schedule/src/types.ts#LegacyScheduleRecord`](#persistence-type-packagesscheduleschedulesrctypestslegacyschedulerecord) |
| `version` | required | `1` |
...
The archive command captured every V4 root and reachable declared type from the isolated pre-V5 checkout, with source line numbers removed. The inventory describes declarations, not every physical codec, plugin payload, or lifecycle rule. pnpm run verify-persistence-formats checks complete format coverage and schema consistency without replaying historical applications.
